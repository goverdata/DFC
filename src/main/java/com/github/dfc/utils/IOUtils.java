...
public class IOUtils {
	...
	/*
	 * public static void copyBytes(InputStream in, OutputStream out,
	 * Configuration conf) throws IOException { copyBytes(in, out,
	...
	/*
	 * public static void copyBytes(InputStream in, OutputStream out,
	 * Configuration conf, boolean close) throws IOException { copyBytes(in,
	 * out, conf.getInt("io.file.buffer.size", 4096), close); }
	 */
	/**
	...
	public static void copyBytes(InputStream in, OutputStream out, long count,
		...
		try {
			while (bytesRemaining > 0) {
				int bytesToRead = (int) (bytesRemaining < buf.length ? bytesRemaining
						: buf.length);
			...
			if (ret == 0) {
				...
				if (b == -1) {
					throw new EOFException(
							"Premature EOF from inputStream after "
